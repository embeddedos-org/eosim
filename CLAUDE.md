...
Applies to every task in `EoSim`.
...
```text
help install install-dev install-all test test-unit test-integration coverage lint format check clean build docker docker-push api docs serve-docs version
```
